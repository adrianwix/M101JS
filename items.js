...
function ItemDAO(database) {
	...
	this.getCategories = async function(callback) {
		...
		try {
			...
			const totalArr = categories.map(obj => obj.num);
	...
	this.getItems = async function(category, page, itemsPerPage, callback) {
		...
		try {
			const itemsPerCategory = await this.db
				.collection("item")
				.find(query)
				.sort({ _id: 1 })
	...
	this.getNumItems = async function(category, callback) {
		...
		try {
			const countPerCategory = await this.db
				.collection("item")
				.find(query)
				.count();
			callback(countPerCategory);
		} catch (e) {
			console.log(e);
		}
	};
