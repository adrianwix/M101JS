...
MongoClient.connect(
  "mongodb://localhost:27017/mongomart",
  function(err, db) {
    "use strict";
    ...
    router.post("/user/:userId/cart/items/:itemId", function(req, res) {
      ...
      cart.itemInCart(userId, itemId, function(item) {
        if (item == null) {
          ...
          cart.updateQuantity(userId, itemId, item.quantity + 1, function(
            userCart
          ) {
            renderCart(userCart);
          });
    ...
    router.post("/user/:userId/cart/items/:itemId/quantity", function(
      req,
      res
    ) {
      "use strict";
